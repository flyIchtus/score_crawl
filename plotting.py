...
for i in range(Length_list_names):
    ...
    res_spectrum_real = pickle.load(open('real3stand_alone_metrics_66048.p', 'rb'))['spectral_compute']
    ...
    for j in range(N_vars):
        ...
        fig.tight_layout()
        plt.savefig('spectral/Spectral_PSD_{}'.format(var_names[j]) +list_names[i]+'.png')
        plt.close()
...
###CORRELATION LENGTH
fake_corr_length_scores = np.zeros((Length_list_names, N_tests, N_vars, 127, 127))
...
for i in range(Length_list_names):
    ...
    for k in range(N_vars) : 
        ...
        ax = grid[k]
        ax.imshow(real_corr[k,:,:], origin ='lower', vmin = fake_corr_length[k].min(), vmax = fake_corr_length[k].max())
        
        cb4 = ax4.cax.colorbar(im4)
        
        
    fig.tight_layout()
    plt.savefig('corr_length/Length_scales_map_comparison'+list_names[i]+'.png')
    plt.close()
...
for i in range(Length_list_names):
    ...
    print(list_names[i] + ': ', np.round(np.mean(SWD_scores[i], axis=0),2), np.round(np.mean(W1_scores[i], axis=0),2),
          np.round(np.mean(W1_scores_c[i], axis=0),2), np.round(mae_corr[i],2), np.round(std_ae_corr[i],2), np.round(max_ae_corr[i],2), np.round(Error_spec[i,0],2),
