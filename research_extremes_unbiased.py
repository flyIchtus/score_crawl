...
for d_idx, dt in enumerate(liste_dates):
    ...
    for ech_idx, ech in enumerate(liste_ech):
        ...
        sample_real =  denorm(np.load(real_dir + 'samples/' + f"{expes['real'][-1]}_{dt}_{ech}.npy",mmap_mode='r').astype(np.float32), Means, Maxs, 0.95)
        for mb_idx in range(N_samples):
            sample = sample0[mb_idx] - sample0[(mb_idx//per_cond) * per_cond : (mb_idx//per_cond) * per_cond + per_cond].mean(axis=0) + sample_real[mb_idx//per_cond]
            ...
            if np.any(inf):
                ...
                if count_bottom <= 20:
                    for var_idx in range(3):
                        ...
                        plt.colorbar()
                        plt.savefig(out_dir + f'sample_bottom_{dt}_{ech}_{var[var_idx]}_{mb_idx}_unbiased.png')
                        plt.close()
            ...
            if np.any(sup):
                ...
                if count_top<=20:
                    for var_idx in range(3):
                        plt.imshow(sample[var_idx], origin='lower',cmap=colors[var_idx], vmin=mins[var_idx], vmax=maxs[var_idx])
                        plt.colorbar()
                        plt.savefig(out_dir + f'sample_top_{dt}_{ech}_{var[var_idx]}_{mb_idx}_unbiased.png')
                        plt.close()

print(count_top, count_bottom)
np.save(out_dir + f"idx_top_{expe}unbiased.npy",idx_top)
np.save(out_dir + f"idx_bottom_{expe}_unbiased.npy",idx_bottom)